...
class MainScreenViewModel(
    ...
    fun benchmark() = viewModelScope.launch {
        ...
        benchMemoryResult?.let { writeLog(it.toString()) }
            ?: writeLog("Error: benchMemory() returned null")

        val benchGgmlMulMatResult = whisperContext?.benchGgmlMulMat(4)
        benchGgmlMulMatResult?.let { writeLog(it.toString()) }
            ?: writeLog("Error: benchGgmlMulMat() returned null")
    ...
    companion object {
        ...
        private var factoryTranslate: Int = 0
        private var factoryLang: String? = null
        private var factoryTranscribePrompt: String? = null
        private var factoryPrintTimestamp: Boolean = true
        private var factoryPlayback: Int = 1
