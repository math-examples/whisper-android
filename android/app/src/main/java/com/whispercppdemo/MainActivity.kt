...
class MainActivity : ComponentActivity() {
    companion object {
        ...
        const val EXTRA_TRANSLATE = "arg4"
        const val EXTRA_LANG = "arg5"
        const val EXTRA_TRANSCRIBE_PROMPT = "arg6"
        const val EXTRA_PRINT_TIMESTAMP = "arg7"
        const val EXTRA_PLAYBACK = "arg8"
        const val EXTRA_DEBUG_LOG_PATH = "arg9"

        const val EXTRA_PENDING_RESULT_INTENT = "pending_result_intent"
        const val EXTRA_TRANSCRIPTION_RESULT = "extra_result_data"
    }

    private var respondToExternalApp = false

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        respondToExternalApp = !intent.getStringExtra(EXTRA_SAMPLE_PATH).isNullOrEmpty()
        ...
        val viewModel: MainScreenViewModel by viewModels {
            MainScreenViewModel.factory(
                ...
                translate = intent.getStringExtra(EXTRA_TRANSLATE)?.toIntOrNull() ?: 0,
                lang = intent.getStringExtra(EXTRA_LANG),
                transcribePrompt = intent.getStringExtra(EXTRA_TRANSCRIBE_PROMPT),
                printTimestamp = intent.getStringExtra(EXTRA_PRINT_TIMESTAMP)?.toIntOrNull() == 1,
                playback = intent.getStringExtra(EXTRA_PLAYBACK)?.toIntOrNull() ?: 1,
        ...
        viewModel.onTaskFinished = { result ->
            ...
            if (respondToExternalApp) {
                try {
                    val pendingResultIntent =
                        intent.getParcelableExtra<PendingIntent>(EXTRA_PENDING_RESULT_INTENT)
