...
// com.whispercpp.whisper.WhisperLib$Companion
// The 00024 is a JNI name mangling sequence that represents the $ character (dollar sign) in the fully qualified name of a nested class.
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_fullTranscribe(
        JNIEnv *env, jobject thiz, jlong context_ptr, jint num_threads, jfloatArray audio_data, jstring prompt_str) { // Updated: Add prompt_str
    UNUSED(thiz);
    struct whisper_context *context = (struct whisper_context *) context_ptr;
    ...
    if (prompt_str != NULL) {
        ...
        LOGI("Using custom transcription prompt: %s", prompt);
    }
    ...
    // Set custom prompt (Whisper API)
    if (prompt != NULL) {
        params.initial_prompt = prompt; // Critical: Pass prompt to Whisper params
    }
